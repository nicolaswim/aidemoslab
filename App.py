...
class StreamlitApp:
    ...
    def initialize_session_state(self):
        ...
        if 'memory' not in st.session_state:
            st.session_state['memory'] = []
        if 'interaction_count' not in st.session_state:  # Track the number of interactions
            st.session_state['interaction_count'] = 0

    def setup_ui(self):
        st.title("Chatbot Conversation")
                # Display popup after 21 interactions
        if st.session_state['interaction_count'] == 20:
            st.info("Hello! You might have noticed that your interaction with ChatGPT has just consumed a whole bottle of water! Are you curious to know how and why this happened? Click the information box below for details.")
    ...
    def on_user_input_change(self):
        ...
        if user_input:  # Ensure there's input before proceeding
            self.chatbot.ask_chatbot(user_input)
            st.session_state['interaction_count'] += 1  # Increment interaction count
            # After chatbot response, refresh the UI
            self.setup_ui()
